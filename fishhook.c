...
// A compact fishhook implementation based on the public-domain-style upstream source.
...
static void rebind_symbols_for_image(struct rebindings_entry *rebindings, const struct mach_header *header, intptr_t slide) {
  ...
  for (uint32_t i = 0; i < ((mach_header_t *)header)->ncmds; i++, cur += cur_seg_cmd->cmdsize) {
